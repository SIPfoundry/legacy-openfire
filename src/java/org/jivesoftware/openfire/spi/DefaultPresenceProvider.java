...
public class DefaultPresenceProvider implements PresenceProvider {
	private static final Logger Log = LoggerFactory
			.getLogger(DefaultPresenceProvider.class);
	...
	public TimePresence loadOfflinePresence(String username) {
		...
		try {
			...
			if (rs.next()) {
				String offlinePresence = DbConnectionManager.getLargeTextField(
						rs, 1);
				if (rs.wasNull()) {
	 ...
	 * {@inheritDoc}
	 */
	public void insertOfflinePresenceIntoDB(String username,
			String offlinePresence, Date offlinePresenceDate) {
		...
		try {
			...
			if (offlinePresence != null) {
				DbConnectionManager
						.setLargeTextField(pstmt, 2, offlinePresence);
			} else {
			...
			Log.error("Error storing offline presence of user: " + username,
					sqle);
		} finally {

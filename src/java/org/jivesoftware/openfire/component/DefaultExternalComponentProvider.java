...
public class DefaultExternalComponentProvider implements ExternalComponentProvider {
	private static final Logger log = LoggerFactory
			.getLogger(DefaultExternalComponentProvider.class);
	 ...
	 * {@inheritDoc}
	 */
	public ExternalComponentConfiguration getConfiguration(String subdomain,
			boolean useWildcard) {
		...
		try {
			...
			while (rs.next()) {
				configuration = new ExternalComponentConfiguration(subdomain,
						false, Permission.valueOf(rs.getString(2)),
						rs.getString(1));
			}
		...
		if (configuration == null && useWildcard) {
			// Check if there is a configuration that is using wildcards for
			// domains
			try {
				// Check if there is a configuration for the subdomain
				con = DbConnectionManager.getConnection();
				...
				while (rs.next()) {
					configuration = new ExternalComponentConfiguration(
							subdomain, true,
							Permission.valueOf(rs.getString(2)),
							rs.getString(1));
				}
			} catch (SQLException sqle) {
				log.error(sqle.getMessage(), sqle);
	...
	public Collection<ExternalComponentConfiguration> getConfigurations(
		...
		try {
			...
			while (rs.next()) {
				...
				subdomain = wildcard ? subdomain.substring(0,
						subdomain.length() - 1) : subdomain;
				configuration = new ExternalComponentConfiguration(subdomain,
						wildcard, permission, rs.getString(3));
	 ...
	 * {@inheritDoc}
	 */
	public void deleteConfigurationFromDB(
			ExternalComponentConfiguration configuration) {
		// Remove the permission for the entity from the database
		Connection con = null;
		PreparedStatement pstmt = null;

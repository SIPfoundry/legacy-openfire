...
public abstract class BasePubSubProvider implements PubSubProvider {
     ...
     */
    protected void savePublishedItem(RetryWrapper wrapper) {
        ...
        synchronized (itemsPending) {
    		LinkedListNode<PublishedItem> itemToReplace = itemsPending.remove(itemKey);
    		if (itemToReplace != null) {
    			itemToReplace.remove(); // remove duplicate from itemsToAdd linked list
    		}
    		LinkedListNode<PublishedItem> listNode = firstPass ? 
    							itemsToAdd.addLast(wrapper.get()) : 
    							itemsToAdd.addFirst(wrapper.get());
    ...
    public void removePublishedItem(PublishedItem item) {
        ...
        synchronized (itemsPending)
    	{
			...
			LinkedListNode<PublishedItem> itemToAdd = itemsPending.remove(itemKey);
			if (itemToAdd != null)
     ...
     */
    protected static class RetryWrapper {
